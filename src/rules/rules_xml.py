...
def classify_xml_log(event):

    def Brute_Force_check(event):
        
        print("DEBUG: EventID =", event["Event"]["System"]["EventID"]["#text"])
        ...
        if len(failed_attempts[key]) >= 10:
            ...
            suspicious_events_xml.append(suspicious_events_step)

            failed_attempts[key] = [failed_attempts[key][-1]]

        print("Processing:", event["Event"]["System"]["EventID"]["#text"])
